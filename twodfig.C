...
void twodfig() {
	...
	TFile* fIn = new TFile ("fout_corr_pp13TeV.root", "read");
	...
	c1->Divide(2, 1);
    c1->cd(1);hHM->Draw("colz");c1->cd(2);hLM->Draw("colz");
  	TCanvas *c2 = new TCanvas ("hHMP", "hHMP", 1);
	c2->Divide(2, 1);  
    c2->cd(1);hDphiHM->Draw();
    c2->cd(2); hDphiLM->Draw();

    TFile* fOut = new TFile ("output/fout_long_range_correlation.root", "recreate");
